...
function SearchResults({ searchInput, setSearchInput }) {
	...
	function getBookData() {
		const url = `https://www.googleapis.com/books/v1/volumes?q=${searchInput.searchBy}${searchInput.searchValue}&maxResults=40
        `;
				...
				<div>
					...
					{books ? (
						...
						<h2 className='did-not-return'>
							Your search did not return any books. Please check your spelling
							or try using a different keyword.
						</h2>
					)}
				</div>
	);
